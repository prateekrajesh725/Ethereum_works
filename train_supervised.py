...
from evaluation.evaluation import eval_node_classification,eval_edge_classification
...
for i in range(args.n_runs):
    ...
    for epoch in range(args.n_epoch):
        ...
        val_auc,val_prec,val_rec = eval_node_classification(tgn, decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
                                           n_neighbors=NUM_NEIGHBORS)
        ...
        logger.info(
            f'Epoch {epoch}: train loss: {loss / num_batch}, val auc: {val_auc},val prec: {val_prec},,val rec: {val_rec}, time: {time.time() - start_epoch}')  # , test auc: {test_auc}
    ...
    best_val_prec = val_precs[early_stopper.best_epoch]
    best_val_rec = val_recs[early_stopper.best_epoch]
    beat_test_auc,beat_test_prec,beat_test_rec = eval_node_classification(tgn, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,
                                            n_neighbors=NUM_NEIGHBORS)
    # else:
    ...
    pickle.dump({
        ...
        "test_prec": beat_test_prec,
        "test_rec": beat_test_rec,
        "train_losses": train_losses,
        "epoch_times": [0.0],
        "new_nodes_val_aps": [],
        "new_node_test_ap": 0,
    ...
    logger.info(f'Run {i}, Bset val auc: {best_val_auc}, test auc: {beat_test_auc},Bset val prec: {best_val_prec}, test prec: {beat_test_prec},test rec: {beat_test_rec}')
    final_val_aucs.append(best_val_auc)
    final_test_aucs.append(beat_test_auc)
    final_val_precs.append(best_val_prec)
    final_test_precs.append(beat_test_prec)
    final_val_recs.append(best_val_rec)
    final_test_recs.append(beat_test_rec)
...
logger.info(f'Final val prec: {np.mean(final_val_precs)} ± {np.std(final_val_precs)}')
logger.info(f'Final test prec: {np.mean(final_test_precs)} ± {np.std(final_test_precs)}')
logger.info(f'Final val rec: {np.mean(final_val_recs)} ± {np.std(final_val_recs)}')
logger.info(f'Final test rec: {np.mean(final_test_recs)} ± {np.std(final_test_recs)}')