...
def preprocess(data_name):
    ...
    edge_label = np.zeros(edge_type.shape)
    #label_edge = np.zeros(edge_type.shape[0])
    ...
    #train_label_edge =  np.concatenate([label_src[train_idx_mask_src], label_dst[train_idx_mask_dst]])
    ...
    test_idx =  np.concatenate([idx[test_idx_mask_src], idx[test_idx_mask_dst]])
    test_label =  np.concatenate([label_src[test_idx_mask_src], label_dst[test_idx_mask_dst]])
    
    # Cast test_idx to integers before using it to index edge_type
    test_id = test_idx.astype(int)
    test_label_edge = edge_type[test_idx]
    ...
    all_train_df = pd.DataFrame({
        ...
        'idx': all_train_idx,
        'edge_label': edge_type[all_train_idx]
    })
    ...
    train_df = pd.DataFrame({
        ...
        'label': train_label,
        'idx': train_idx,
        'edge_label': edge_type[train_idx]
    })
    ...
    test_df = pd.DataFrame({
        ...
        'label': test_label,
        'idx': test_idx,
        #'edge_label': test_label_edge
        'edge_label': edge_type[test_idx]
    })
