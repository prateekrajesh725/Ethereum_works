...
node_features, edge_type, full_data, train_data, val_data, test_data = get_data(DATA,
...
train_rand_sampler = RandEdgeSampler(train_data.sources, train_data.destinations, seed=args.seed)
...
for i in range(args.n_runs):
    ...
    tgn = tgn.to(device) # torch.nn.DataParallel(tgn.to(device), device_ids=gpu_list, output_device=gpu_list[0])
    ...
    for epoch in range(NUM_EPOCH):
        ...
        val_ap, val_auc = eval_edge_prediction(model=tgn,
                                               ...
                                               data=val_data,
                                               n_neighbors=NUM_NEIGHBORS, batch_size=args.bs)
        if USE_MEMORY:
        ...
        logger.info('Run {}, epoch: {} took {:.2f}s'.format(i, epoch, total_epoch_time))
